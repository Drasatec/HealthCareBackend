...
[ApiController]
[ApiVersion("1.0")]
public class DoctorsDegreeController : ControllerBase
{
    private readonly IUnitOfWork Data;

    public DoctorsDegreeController(IUnitOfWork data)
    {
        Data = data;
    }
    ...
    public async Task<IActionResult> AddSingle([FromForm] DoctorsDegree model)
    {
    ...
    public async Task<IActionResult> GetById([FromQuery] int id, string? lang)
    {
        ...
        Expression<Func<DoctorsDegree, object>>? filterExpression;
        if (lang != null)
        {
            filterExpression = inc => inc.DoctorsDegreesTranslations.Where(l => l.LangCode == lang);
        }
        else
            filterExpression = inc => inc.DoctorsDegreesTranslations;
    ...
    public async Task<IActionResult> GetAllNames([FromQuery] string? lang, int? page, int? pageSize)
    {
        ...
        var result = await Data.Generic.GenericReadAll<DoctorsDegreesTranslation>(t => t.LangCode.Equals(lang), null, page, pageSize);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> Search([FromQuery] string? searchTerm,  string? name, int? page, int? pageSize,  string? lang)
    {
        ...
        {
            return Ok(await Data.Generic.GenericSearchByText<DoctorsDegreesTranslation>(t => t.DegreeName.Contains(name), null, page, pageSize));
        }
        else if (!string.IsNullOrEmpty(searchTerm) && lang != null)
        {
            Expression<Func<DoctorsDegree, bool>> filter = f => f.DoctorsDegreesTranslations.Any(t => t.DegreeName.Contains(searchTerm));
            Expression<Func<DoctorsDegree, object>> include = i => i.DoctorsDegreesTranslations.Where(l => l.LangCode == lang);
    ...
    public async Task<IActionResult> UpdateSingleWithImage([FromForm] DoctorsDegree model)
    {
        Response response;
    ...
    public async Task<IActionResult> Add_EditTranslations([FromForm] List<DoctorsDegreesTranslation> translations)
    {
        Response response;
    ...
    {
        Expression<Func<DoctorsDegreesTranslation, bool>> expression;

        if (parentId.HasValue)
            expression = t => t.DoctorDegreeId.Equals(parentId);
        else
    ...
    {
        var res = await Data.Generic.GenericDelete<DoctorsDegree>(t => t.Id.Equals(id));
        if (res.Success)
            return Ok(res);
        return BadRequest(res);
    }
