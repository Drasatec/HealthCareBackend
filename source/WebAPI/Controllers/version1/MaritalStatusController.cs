...
public class MaritalStatusController : ControllerBase
{
    ...
    public MaritalStatusController(IUnitOfWork data)
    {
        Data = data;
    }
    ...
    public async Task<IActionResult> GetById([FromQuery] int id, string? lang)
    {
        ...
        Expression<Func<MaritalStatus, object>>? filterExpression;
        if (lang != null)
        {
            filterExpression = inc => inc.MaritalStatusTranslations.Where(l => l.LangCode == lang);
        }
        else
            filterExpression = inc => inc.MaritalStatusTranslations;
    ...
    public async Task<IActionResult> GetAllNames([FromQuery] string? lang, int? page, int? pageSize)
    {
        ...
        var result = await Data.Generic.GenericReadAll<MaritalStatusTranslation>(t => t.LangCode.Equals(lang), null, page, pageSize);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> GetAll([FromQuery] int? pageSize, int? page, string? lang)
    {
        ...
        var result = await Data.Generic.GenericReadAllWihInclude<MaritalStatus>(null, o => o.Id, inc => inc.MaritalStatusTranslations.Where(l => l.LangCode == lang), page, pageSize);
    ...
    public async Task<IActionResult> Search([FromQuery] string? searchTerm, string? name, int? page, int? pageSize, string? lang)
    {
        ...
        {
            return Ok(await Data.Generic.GenericSearchByText<MaritalStatusTranslation>(t => t.Name.Contains(name), null, page, pageSize));
        }
        else if (!string.IsNullOrEmpty(searchTerm) && lang != null)
        {
            Expression<Func<MaritalStatus, bool>> filter = f => f.MaritalStatusTranslations.Any(t => t.Name.Contains(searchTerm));
            Expression<Func<MaritalStatus, object>> include = i => i.MaritalStatusTranslations.Where(l => l.LangCode == lang);
    ...
    public async Task<IActionResult> UpdateSingleWithImage([FromForm] MaritalStatus model)
    {
        Response response;
    ...
    public async Task<IActionResult> Add_EditTranslations([FromForm] List<MaritalStatusTranslation> translations)
    {
        Response response;
    ...
    {
        Expression<Func<MaritalStatusTranslation, bool>> expression;

        if (parentId.HasValue)
            expression = t => t.MaritalId.Equals(parentId);
        else
    ...
    {
        var res = await Data.Generic.GenericDelete<MaritalStatus>(t => t.Id.Equals(id));
        if (res.Success)
            return Ok(res);
        return BadRequest(res);
    }
