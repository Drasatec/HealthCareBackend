...
public interface IMedicalSpecialtyRepository : IGenericRepository
{
    ...
    Task<AllMedicalSpecialtyDto?> ReadAll(int? baseid, bool? isBaseActive, string? status, string? lang, int? pageSize, int? page);
    Task<MedicalSpecialtyDto?> ReadById(int? Id, string? lang = null);
    Task<AllMedicalSpecialtyDto?> SearchByNameOrCode(bool? isActive, string searchTerm, string lang, int? page, int? pageSize);
