...
public interface IBuildingRepository : IGenericRepository
{
    ...
    Task<AllBuildingDto?> ReadAll(int? parentId, bool? isHosActive, string? status, string? lang, int? pageSize, int? page);
    Task<AllBuildingDto?> SearchByNameOrCode(bool? isActive, string searchTerm, string lang, int? page, int? pageSize);
}
