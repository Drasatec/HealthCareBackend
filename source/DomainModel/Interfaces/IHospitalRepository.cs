...
public interface IHospitalRepository : IGenericRepository
{
    ...
    Task<AllHospitalsDto?> ReadAllHospitals(string? status, string? lang, int? pageSize, int? page);
    Task<List<HospitalTranslation>> SearchByHospitalName(string name);
    Task<AllHospitalsDto?> SearchByHospitalNameOrCode(bool? isActive, string searchTerm, string lang, int? page, int? pageSize);
