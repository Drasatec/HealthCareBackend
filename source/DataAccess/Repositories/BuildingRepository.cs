...
public class BuildingRepository : GenericRepository, IBuildingRepository
{
    ...
    #region Update
    public async Task<Response<BuildingDto?>> UpdateHospital(BuildingDto dto, int id, Stream? image = null)
    {
    ...
    public async Task<AllBuildingDto?> ReadAll(bool? isHosActive ,string isActive, string? lang, int page = 1, int pageSize = 10)
    {
    ...
    public async Task<AllBuildingDto?> SearchByNameOrCode(string searchTerm, string lang = "ar", int page = 1, int pageSize = 10)
    {
        int skip = Helper.SkipValue(page, pageSize);
        IQueryable<HosBuilding> query = Context.HosBuildings;
